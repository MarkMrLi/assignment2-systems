...
def benchmark_one_step(
    ...
    inference_ctx = torch.inference_mode() if fwd_only else nullcontext()
    ...
    optimizer = None if fwd_only else AdamW(params=model.parameters())
    ...
    for step in range(10):
        ...
        try:
            if fwd_only:
                ...
                forward_duration = (timeit.default_timer() - start_forward_time) * 1000
                fwd_times.append(forward_duration)
                if enable_pf_mem:
                    results_dir = pathlib.Path("results/memory")
                    results_dir.mkdir(parents=True, exist_ok=True)
                    forward_pickle_path = (
                        results_dir
                        / f"memory_snapshot_forward_only_dmodel_{d_model}_ctx_{context_length}_bs_{batch_size}_dtype_{dtype_tag}.pickle"
                    )
                    torch.cuda.memory._dump_snapshot(str(forward_pickle_path))
            else:
                ...
                with autocast_ctx, torch.cuda.nvtx.range("Backward"):
                    ...
                    if device == "cuda":
                        torch.cuda.synchronize()
                    backward_duration = (timeit.default_timer() - start_backward_time) * 1000
                ...
                if enable_pf_mem:
                    results_dir = pathlib.Path("results/memory")
                    results_dir.mkdir(parents=True, exist_ok=True)
                    fullstep_pickle_path = (
                        results_dir
                        / f"memory_snapshot_full_step_dmodel_{d_model}_ctx_{context_length}_bs_{batch_size}_dtype_{dtype_tag}.pickle"
                    )
                    torch.cuda.memory._dump_snapshot(str(fullstep_pickle_path))
        finally:
    ...
    return {
        ...
        "bwd_mean": np.mean(bwd_times) if bwd_times else 0.0,
        "bwd_std": np.std(bwd_times) if bwd_times else 0.0,
        "total_mean": np.mean(fwd_times) + (np.mean(bwd_times) if bwd_times else 0.0),
    }
...
def run_benchmark(
    enable_mix_precision: bool,
    mix_type: str,
    pf_mem: bool,
    fwd_only: bool,
    model_size_filter: str = "all",
    context_len_filter: str = "all",
):
    ...
    table_markdown = df.to_markdown(index=False) or ""
...
def main():
    ...
    parser.add_argument("--fwd_only", action="store_true", help="Inference forward only")
    parser.add_argument(
        "--model_size",
        type=str,
        default="all",
    ...
    run_benchmark(args.mix, args.mix_dtype, args.memory, args.fwd_only, args.model_size, args.context_len)
    return
