...
## Torch Compile Benchmarking (Attention)
- Extended `cs336_systems/benchmark_attention.py` to benchmark `torch.compile` by compiling `scaled_dot_product_attention` and saving results to `results/profile_attention/torch_compile.md`.
- Observed regime shift with sequence length:
  - Small seq_len: compiled can be slower (compile overhead + dispatch overhead dominates).
  - Large seq_len (>= 4096): compiled improves forward (~1.4-1.6x) and backward (~1.7x) on average.
- Memory interpretation:
  - `add_mem_MB` is nearly identical between vanilla and compiled, consistent with still materializing the full `[B, N, N]` attention matrices needed for backward.
  - `mem_max_MB` drops substantially at long seq_len (roughly ~2x), consistent with fewer/lower-footprint transient intermediates (fusion/layout/workspace effects).
- Measurement caveat: compile cache and first-call compile time were not explicitly measured; peak memory may include compile-time allocations unless isolated.

## Unresolved Issues (Next)
1. End-to-end Transformer benchmarking with `torch.compile(model)` (forward-only vs full train step: forward+backward+optimizer) for the `torch_compile` problem part (b).
2. Add a measurement plan to separate first-iteration compile cost from steady-state runtime (e.g., record iteration 1 separately and/or run longer warmup).
